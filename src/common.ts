...
export function hostNew(stack: any[], ...args: any[]) {
  ...
  return promiseAll(args.map((a) => {
    if (isList(a)) {
      a = untick(a);
      if (a.length !== 2) {
        return Promise.reject(new Error("new - invalid argument, lists must be of length 2 " + a));
      }
    ...
    if (isObject(a)) {
      return Promise.reject(new Error("new - invalid argument, objects cannot be passed directly as arguments"));
    }
...
export function newOf(stack: any[], typeInfo: any, ...args: any[]) {
  ...
  return evalHost(stack, typeInfo)
    .then((_typeInfo) => {
      typeInfo = _typeInfo;
      if (!isObjectInfo(typeInfo)) {
        throw new Error("newOf - invalid first argument. Expected objectInfo, given " + typeInfo);
      }
      return hostNew(stack, ...args);
    })
    .then((_newObject) => {
      newObject = _newObject;
      const ti: IObjectInfo = typeInfo;
...
export function setr(stack: any[], ...args: any[]) {
  if (args.length < 3) {
    throw new Error("setr - path must have at least 3 items, root, field, and value. Given:" + args);
  }
  const rootExpr = args.shift();
...
function hostReduce(stack: any[], items: any[], memoSym: any, iterSym: any, ...body: any[]) {
  ...
  return f.then((_f: any) => {
    ...
    .then((_items: any[]) => {
      ...
      items.reverse();
      let memo: any;
      if (isNvp(memoSym)) { memo = memoSym.value; } else { memo = items.pop(); }
      function next(item: any): any {
        return evalHost(stack, ["`", f, item, memo])
          .then((r) => {
            memo = r;
            if (items.length) { return next(items.pop()); } else { return memo; }
          });
      }
...
function hostFor(stack: any[], iterArgs: any[], ...body: any[]) {
  ...
  const iterSym = untick(iterArgs.shift());
...
export function spread(stack: any[], expr: any[], spreadArg: any, ...addArgs: any[]) {
  return evalHost(stack, spreadArg)
    .then((spreadArg) => {
      // TODO insead of throwing an error we could try to be smart
      //      but not doing that now to avoid over complicating things
      if (!isList(expr)) { 
        throw new Error("spread - first arg is expected to be an expression. Given: " + expr); 
      }
