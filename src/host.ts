...
export async function validateArg(stack:any[], f:Fn, param:ParamInfo, arg:any) {
  try {
...
export async function mapArgs(stack:any[], args:any[], f:any) {
  ...
  // named values
  const namedValues = <Nvp[]> args.filter(isNvp);
  ...
  // values by position
  let orderedValues = args.filter(x => !isNvp(x));
