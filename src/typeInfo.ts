...
export interface IUnionTypeInfo {
  kind: "UnionTypeInfo";
  id?: string;
  name?: string;
  typeInfos: TypeInfo[];
...
export type TypeInfo = IValueInfo | IObjectInfo | IListInfo | IUnionTypeInfo | IFnInfo;
...
export function makeFn(
  name?: string, params: Array<string | IParamInfo>= [], returnType?: TypeInfo, body: any[]= [], closure?: any[]): Fn {
...
export function typeDisplayStr(x: TypeInfo= Any) {
  if (x.name) { return x.name; } else { return x.kind; }
}
...
export function isOptionalParam(p: IParamInfo) {
  return p.nullable || (p.defaultValue !== undefined && p.defaultValue !== null);
}

export  function validataeParamsFit(srcParams: IParamInfo[], destFn: IFnInfo) {
...
export function fieldInfo(name: string, typeInfo?: TypeInfo, nullable: boolean= false, defaultValue?: any): IFieldInfo {
  return {
    // kind: 'FieldInfo',
    name,
    typeInfo,
...
export function objectInfo(name?: string, fields?: Array<IFieldInfo | string>): IObjectInfo {
...
export function listInfo(
  name?: string, listType?: TypeInfo, itemTypes?: TypeInfo[], minLength?: number, maxLength?: number): IListInfo {
  return {
    kind: "ListInfo",
    name,
    listType,
...
export function unionTypeInfo(name?: string, ...typeInfos: TypeInfo[]): IUnionTypeInfo {
  return {
    kind: "UnionTypeInfo",
    name,
    typeInfos,
...
export function paramInfo(
  name: string, typeInfo?: TypeInfo, nullable: boolean= false, defaultValue?: any, isRest?: boolean): IParamInfo {
  return {
    // kind: 'ParamInfo',
    name,
    typeInfo,
...
export function fnInfo(name?: string, params?: Array<string | IParamInfo>, returnType?: TypeInfo): IFnInfo {
...
export function newObject(info: IObjectInfo= AnyObj, ...args: any[]) {
  const o: any = {};
  if (info.fields) { info.fields.forEach((f) => {
    o[f.name] = args.shift();
    if (o[f.name] === undefined) {
      if (f.defaultValue !== undefined) { o[f.name] = copy(f.defaultValue); } else if (f.nullable) { o[f.name] = null; } else { throw new Error(`${f.name} - required but no argument supplied`); }
    }
  });
  }
...
export function newStruct(...args: INvp[]) {
  const o: any = {};
