import { clone, isDate, isEqual, isFunction, isHtml, isList, isNvp, isObject, isString, keys, union } from "./common";
...
export function toJSON(obj: any) {
  ...
  function recurse(obj: any) {
      ...
      refCount++;
      const newRef = "__duplicate_ref_" + (isList(obj) ? "ary_" : "obj_") + refCount;
      const nObj: (any[] | any) = isList(obj) ? [] : {};
      knownObjs.push(obj);
      objRefs.push(newRef);
      newObjs.push(nObj);
  ...
  function recurse(obj: any) {
      ...
      if (isString(obj)) {
          ...
          if (obj.match(/^__REGEXP /)) {
              const m: any = obj.split("__REGEXP ")[1].match(/\/(.*)\/(.*)?/);
          ...
          if (obj.match(/^__HTML /)) {
              // @ts-ignore
              if (typeof $ !== "undefined") { return $(obj.substring(7))[0]; } else { return obj; }
          }
          if (obj === "__WINDOW") {
              return window;
          }
...
export function cleanCopyList(l: any[]): any[] {
  ...
  function cleanCopy(l: any[]): any[] {
      ...
      for (let i = 0; i < l.length; i++) {
          if (isList(l[i])) { newL[i] = cleanCopyList(l[i]); } else {
              newL[i] = l[i];
...
export 	function diff(main: any, second: any) {
  ...
  function diffLvl(main: any, second: any, prefix: string) {
      const names = union(keys(main), keys(second));
      for (const k of names) {
          const v1 = main[k];
          const v2 = second[k];
          if (!isEqual(v1, v2)) {
              if (typeof v1 == "object" && typeof v2 == "object") { diffLvl(v1, v2, prefix + k + "."); } else { diffs[prefix + k] = [String(v1), String(v2)]; }
          }
      }
  }
  diffLvl(main, second, "");
