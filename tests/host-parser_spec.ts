...
describe("parseHost", () => {
  ...
  it("should throw an error with the starting line and column if possible", async () => {
    $parseHost([], "\n\n\na b c d: ☺").should.be
      .rejectedWith("parse error at line 4, col 8:\na b c d: ☺\nno parsers are proceeding");
  });

  describe("custom parsers", () => {
    it("should pick up and use custom parsers in the stack", async () => {
      const parseSmilyface = parser("parserSmilyface", pi => {
        if (pi.peek() !== "☺") { return; }
        pi.clist.push("`makeASmily");
        pi.i++;
        return true;
      });
      await $parseHost([ { parseSmilyface } ], "☺ 1").then(cleanCopyList).then((ast) =>
...
`).then(cleanCopyList);
  ...
  //   it("should set the tabSize of the current parser", async () => {
