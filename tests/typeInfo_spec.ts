import { newid, nvp } from "../src/common";
import { Any, AnyList, AnyObj, Bool, DT, fieldInfo, fnInfo, Id, IParamInfo, isMeta, isOptionalParam,
        listInfo, makeFn, match, meta, Null, Num, objectInfo, paramInfo, Str,
        typeFits, typeOf, Undefined, unionTypeInfo, validataeParamsFit, validate, valueInfo } from "../src/typeInfo";
...
describe("host TypeInfo", () => {
  ...
  describe("user defined types type", () => {
    ...
    context("with an object type", () => {
      it("should throw an error if a required field is not present", () => {
        should(() => validate(stack, {}, User).should.be)
          .throw(".username[Username] is invalid, value: undefined[Undefined] Error: undefined did not match Str");
      });

      it("should throw an error if a field value is invalid", () => {
        should(() => validate(stack, { username: 1 }, User))
          .throw(".username[Username] is invalid, value: 1[Num] Error: 1 did not match Str");
        should(() => validate(stack, { username: "1aa" }, User))
          .throw(".username[Username] is invalid, value: 1aa[Str] Error: must start with a letter");
      });
    ...
    it.skip("should still work after being stringified and parsed", () => {
      ...
      should(() => validate(stack, { username: 1 }, UserOverTheWire))
        .throw(".username[Username] is invalid, value: 1[Num] Error: 1 did not match Str");
      should(() => validate(stack, { username: "1aa" }, UserOverTheWire))
        .throw(".username[Username] is invalid, value: 1aa[Str] Error: must start with a letter");
      const user = { username: "aaa", name: "john" };
  ...
  describe("validataeParamsFit", () => {
    ...
    it("should allow trailing params to fit in rest param of same type", () => {
      const srcParams: IParamInfo[] = [paramInfo("n1"), paramInfo("n2")];
      const fni = fnInfo("Add", [paramInfo("nums", undefined, false, undefined, true)]);
      validataeParamsFit(srcParams, fni).should.equal(true);
    });
    ...
    it("should not allow mismatching types", () => {
      const srcParams: IParamInfo[] = [paramInfo("n", Num)];
